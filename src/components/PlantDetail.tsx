...
import { ArrowLeft, Sun, Zap, Battery, PlugZap, ChevronDown, Plus, Trash2, Download, FileText, Loader2 } from 'lucide-react';
...
export function PlantDetail({ plantId, onBack, onDeleted }: PlantDetailProps) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setInverters(inv); setStorages(sto); setChargers(chg);
      const urlMap: Record<string, string> = {};
      await Promise.all(pho.map(async (photo) => {
        try { urlMap[photo.id] = await getPhotoUrl(photo.storage_path); } catch { /* skip */ }
      }));
      setPhotoUrls(urlMap);
  ...
  const handleExport = async () => {
    if (!plant) return;
    setExporting(true);
    try { await exportPlantArchive(plant, panels, photos, inverters, storages, chargers); }
    catch { /* skip */ } finally { setExporting(false); }
  };
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      saveAs(blob, `${sanitizeFileName(plant.owner_name)}_${sanitizeFileName(plant.address)}.pdf`);
    } catch { /* skip */ } finally { setExportingPdf(false); }
  };

  const handleDeletePlant = async () => {
    if (!plant) return;
    try { await deletePlant(plant.id); onDeleted(); } catch { /* skip */ }
  };

  if (loading) {
    return <div className="flex items-center justify-center h-full"><Loader2 className="animate-spin text-slate-400" size={32} /></div>;
  }
  if (!plant) {
    return (
      <div className="p-8 text-center">
        <p className="text-slate-500">Impianto non trovato.</p>
        <button onClick={onBack} className="mt-4 text-lime-600 font-medium text-sm">Torna alla dashboard</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto pb-24">
      ...
      <PlantTechnicalDetails plant={plant} />
      ...
      <EquipmentSection title="Inverter" icon={Zap} items={inverters} expanded={invertersExpanded}
        onToggle={() => setInvertersExpanded((v) => !v)}
        ...
        renderItem={(inv, index) => ({
          ...
          brandOptions: inverterOpts.brands, modelOptionsFor: inverterOpts.modelsFor,
          onEnsureBrand: inverterOpts.ensureBrand, onEnsureModel: inverterOpts.ensureModel,
          onChange: (fi, val) => setInverters((prev) => prev.map((it, i) => i !== index ? it : { ...it, [fieldKeys[fi]]: val })),
          onSave: async () => {
            if (!inv.brand && !inv.model && !inv.code) return;
            if (inv.id) { await updateInverter(inv.id, { brand: inv.brand, model: inv.model, code: inv.code }); }
            else { const c = await createInverter({ plant_id: plantId, brand: inv.brand, model: inv.model, code: inv.code, sort_order: index }); setInverters((prev) => prev.map((it, i) => i === index ? c : it)); }
          },
          onDelete: async () => { if (inv.id) await deleteInverter(inv.id); setInverters((prev) => prev.filter((_, i) => i !== index)); },
        })}
      />

      <EquipmentSection title="Accumuli" icon={Battery} items={storages} expanded={storagesExpanded}
        onToggle={() => setStoragesExpanded((v) => !v)}
        ...
        renderItem={(sto, index) => ({
          ...
          brandOptions: storageOpts.brands, modelOptionsFor: storageOpts.modelsFor,
          onEnsureBrand: storageOpts.ensureBrand, onEnsureModel: storageOpts.ensureModel,
          ...
          onSave: async () => {
            if (!sto.brand && !sto.model && !sto.code && sto.power_kw == null) return;
            if (sto.id) { await updateStorage(sto.id, { brand: sto.brand, model: sto.model, code: sto.code, power_kw: sto.power_kw }); }
            else { const c = await createStorage({ plant_id: plantId, brand: sto.brand, model: sto.model, code: sto.code, power_kw: sto.power_kw, sort_order: index }); setStorages((prev) => prev.map((it, i) => i === index ? c : it)); }
          },
          onDelete: async () => { if (sto.id) await deleteStorage(sto.id); setStorages((prev) => prev.filter((_, i) => i !== index)); },
        })}
      />

      <EquipmentSection title="Colonnine" icon={PlugZap} items={chargers} expanded={chargersExpanded}
        onToggle={() => setChargersExpanded((v) => !v)}
        ...
        renderItem={(chg, index) => ({
          ...
          onSave: async () => {
            if (!chg.brand && !chg.model && !chg.code) return;
            if (chg.id) { await updateCharger(chg.id, { brand: chg.brand, model: chg.model, code: chg.code }); }
            else { const c = await createCharger({ plant_id: plantId, brand: chg.brand, model: chg.model, code: chg.code, sort_order: index }); setChargers((prev) => prev.map((it, i) => i === index ? c : it)); }
          },
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <button onClick={() => setPanelsExpanded((v) => !v)} className="flex items-center gap-2 font-semibold text-slate-900 hover:text-blue-900 transition-colors">
            <Sun size={18} /> Pannelli ({panels.length})
            <ChevronDown size={18} className={`transition-transform ${panelsExpanded ? '' : '-rotate-90'}`} />
          </button>
          <button onClick={() => setShowPanelForm(true)} className="flex items-center gap-1.5 bg-blue-900 hover:bg-blue-800 text-white text-sm font-medium px-3 py-2 rounded-lg transition-colors">
            ...
            <div className="space-y-3">
              {panels.map((panel, index) => (
                <PanelRow key={panel.id} panel={panel} index={index}
                  onUpdate={(serial, notes) => updatePanel(panel.id, { serial_number: serial, notes }).then(() => undefined)}
      ...
      <PhotoGrid plantId={plantId} panels={panels} photos={photos} photoUrls={photoUrls}
        onUploaded={loadAll}
      ...
      {showPanelForm && (
        <PanelFormModal onClose={() => setShowPanelForm(false)}
          onSave={async (serial, notes) => {
            const created = await createPanel({ plant_id: plantId, serial_number: serial, position_label: null, notes });
